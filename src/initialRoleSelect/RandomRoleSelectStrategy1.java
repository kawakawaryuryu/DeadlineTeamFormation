 ...
 * 完全にランダムにタスクを割り当てる
 * （エージェントのリソースを未知として）
 * リーダの期待報酬 = タスクの全報酬 × 欲張り度
 * メンバの期待報酬 = サブタスクのリソース × 報酬期待度
 */
public class RandomRoleSelectStrategy1 implements InitialRoleSelectStrategy {
	...
	public double calculateExpectedLeaderReward(Agent agent, Task task) {
		double leader_reward = (double)task.getTaskRequireSum() * agent.getGreedy();	//リーダ時の期待報酬の計算
	...
	public double calculateExpectedMemberReward(Agent agent, OfferMessage message) {
		double member_reward = (double)message.getSubTask().getRequireSum() * agent.getExpectedReward(message.getFrom());
