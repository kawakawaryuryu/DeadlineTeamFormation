 ...
 * リーダに割り当てるサブタスクは一つだけ
 * 1tickごとの期待報酬
 * 報酬期待度は1tickごとの獲得報酬で学習
 * リーダの期待報酬 = (タスクの全報酬 / タスク処理想定時間) × 欲張り度
 * タスク処理想定時間 = タスクのリソース和 / チーム履歴の平均リソース
 * メンバの期待報酬 = (サブタスクのリソース / サブタスク処理時間) × 報酬期待度
 */
public class RandomRoleSelectStrategy4 implements InitialRoleSelectStrategy {
...
//		System.out.println("エージェント" + agent + "がサブタスク" + subtask + "の処理にかかる時間 = " + executeTime);
	...
	public String toString(){
		return "ランダムな戦略（1tickごとの報酬期待度で学習＋リーダに割り当てるサブタスクは一つだけ）";
	}
