...
/**
 * 報酬期待度を
 * (サブタスクの処理時間 / チームの処理時間)で学習
 */
public class LearningExpectedRewardPerTurnAgent3 extends Agent {
	 ...
	 * @param ability
	 */
	public LearningExpectedRewardPerTurnAgent3(int id, int[] ability) {
		super(id, ability);
	}
...
//		System.out.print(this + " の " + you + " に対する報酬期待度：（前） = " + expectedReward[you.id]);
		expectedReward[you.id] = Constant.LEARN_RATE_REWARD * ((double)getExecuteStateTime() / (double)executeTime) + (1.0 - Constant.LEARN_RATE_REWARD) * expectedReward[you.id];	//報酬期待度の更新
//		System.out.println(" / （後） = " + expectedReward[you.id]);
	...
	public String getAgentType(){
		return "(サブタスクの処理時間 / チームの処理時間)で学習";
	}
