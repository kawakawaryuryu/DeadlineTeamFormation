...
/**
 * メンバの中から割り当てが決まらなかったサブタスクの再割り当てを行う
 */
public class LeaderStateStrategy1 implements LeaderStrategy {
	
//	private static Random sort_random3 = new Random(221);	//221メンバ候補を一端並び替えるときのランダムインスタンス
//	private static Random epsilon_greedy_random3 = new Random(223);	//223ε-greedyのときのランダムインスタンス
//	private static Random select_random4 = new Random(227);	//227ランダムにメンバを決めるときのランダムインスタンス
//	private static Random sort_random2 = new Random(229);	//229配列ソート時のランダムインスタンス
...
//				System.out.println(subtask + "を処理するメンバを " + member + " のエージェントとしました");
		...
		if(leader.getLeaderFields().notAssignedSubTask.isEmpty() == false){
			double[] trustOfTrueAgent = new double[leader.getLeaderFields().trueAgentList.size()];	//ソート後のエージェントを保持する配列
			for(int i = 0; i < leader.getLeaderFields().trueAgentList.size(); i++){
				trustOfTrueAgent[i] = leader.getTrust(leader.getLeaderFields().trueAgentList.get(i));
			}
			
//			System.out.println("仮チームのエージェントを提案受託期待度でソート");
			Agent[] sortedTrueAgents = getSortArrayOrder(trustOfTrueAgent ,leader.getLeaderFields().trueAgentList);	//仮チームのエージェントを提案受託期待度でソート
			...
			for(SubTask subtask : leader.getLeaderFields().notAssignedSubTask){
				...
				for(int i = 0; i < sortedTrueAgents.length; i++){
					int taskDeadline = subtask.getDeadline() - sortedTrueAgents[i].getExecuteTime() - 1 * Constant.WAIT_TURN;
					if(isExecuteSubTask(sortedTrueAgents[i], subtask, taskDeadline) == true){
//						System.out.println(subtask + " を処理するメンバを " + sortedTrueAgents[i] + " のエージェントにしました");
						sortedTrueAgents[i].willBeExecutedSubTaskList.add(subtask);	//メンバが処理する予定のサブタスクを追加
						sortedTrueAgents[i].addExecuteTime(calculateExecuteTime(sortedTrueAgents[i], subtask));	//エージェントのタスク処理時間を保持
						
						/* メンバーリストに入っていなかったらメンバーリストに入れる */
						if(leader.getLeaderFields().memberList.contains(sortedTrueAgents[i]) == false){
							leader.getLeaderFields().memberList.add(sortedTrueAgents[i]);
						}
...
//		System.out.println("エージェント" + agent + "がサブタスク" + subtask + "の処理にかかる時間 = " + executeTime);
	...
	public String toString(){
		return "メンバの中で信頼度の大きい順に再割り当てを行う";
	}
}
